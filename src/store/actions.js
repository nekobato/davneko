...
export const addDir2Queue = ({ commit, state }) => {
  api
    .fetchDirRecursive(state.depth.files[state.depth.files.length - 1])
    .then(files => {
...
export const removeQueues = ({ commit }) => {
  commit(types.REMOVE_QUEUES_ALL);
};

export const play = ({ commit }) => {
  commit(types.PLAYER_PLAYED);
};

export const pause = ({ commit }) => {
  commit(types.PLAYER_PAUSED);
};
...
export const playPrev = ({ commit, state }) => {
  ...
  if (state.playlist.queues.length <= 1) {
    // nothing to do
  } else if (state.playlist.queues[maxIndex - 1]) {
    commit(types.PLAY_QUEUE, state.playlist.queues[maxIndex - 1], maxIndex - 1);
  } else {
...
export const onPlay = store => {
  store.commit(types.AUDIO_PLAYED);
};

export const onPause = store => {
  store.commit(types.AUDIO_PAUSED);
};

export const timeUpdate = (store, currentTime) => {
  store.commit(types.AUDIO_TIME_UPDATED, currentTime);
};

export const onEnded = store => {
  store.commit(types.AUDIO_ENDED);
};
