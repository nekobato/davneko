import { Inject, Injectable } from '@nestjs/common';
import { BetterSQLite3Database } from 'drizzle-orm/better-sqlite3';
import { schema } from 'src/db/schema';
import { eq, sql } from 'drizzle-orm';
import { generateId } from 'src/utils/crypt';

@Injectable()
export class PlaylistRepository {
  constructor(@Inject() private db: BetterSQLite3Database<typeof schema>) {}

  async findPlaylistsByUserId(userId: string) {
    return this.db.query.playlist.findMany({
      where(fields, operators) {
        return operators.eq(fields.userId, userId);
      },
    });
  }

  async findPlaylistById(id: string) {
    return this.db.query.playlist.findFirst({
      where(fields, operators) {
        return operators.and(operators.eq(fields.id, id));
      },
    });
  }

  async findPlaylistAudioByPlaylistId(playlistId: string) {
    return this.db.query.playlistAudio.findMany({
      where(fields, operators) {
        return operators.and(operators.eq(fields.playlistId, playlistId));
      },
    });
  }

  async create(userId: string, name: string) {
    return this.db
      .insert(schema.playlist)
      .values({
        id: generateId(),
        userId,
        name,
      })
      .returning();
  }

  async updatePlaylist(
    id: string,
    data: {
      name: string;
    },
  ) {
    return this.db
      .update(schema.playlist)
      .set(data)
      .where(eq(schema.playlist.id, id))
      .returning();
  }

  async deleteQueue(id: string) {
    return this.db.delete(schema.queue).where(eq(schema.queue.id, id));
  }

  async updatePlayedAt(id: string) {
    return this.db
      .update(schema.queue)
      .set({
        playedAt: new Date().toISOString(),
      })
      .where(eq(schema.queue.id, id))
      .returning();
  }

  async reOrderQueueToEarlier(
    userId: string,
    queueId: string,
    newOrder: number,
  ) {
    return this.db.transaction(() => {
      this.db
        .update(schema.queue)
        .set({
          orderInQueue: newOrder,
        })
        .where(eq(schema.queue.userId, userId))
        .execute();
      this.db
        .update(schema.queue)
        .set({
          orderInQueue: sql`order_in_queue + 1`,
        })
        .where(
          sql`order_in_queue >= ${newOrder} AND user_id = ${userId} AND id != ${queueId}`,
        )
        .execute();
    });
  }

  async reOrderQueueToLater(userId: string, queueId: string, newOrder: number) {
    return this.db.transaction(() => {
      this.db
        .update(schema.queue)
        .set({
          orderInQueue: newOrder,
        })
        .where(eq(schema.queue.userId, userId))
        .execute();
      this.db
        .update(schema.queue)
        .set({
          orderInQueue: sql`order_in_queue - 1`,
        })
        .where(
          sql`order_in_queue <= ${newOrder} AND user_id = ${userId} AND id != ${queueId}`,
        )
        .execute();
    });
  }
}
